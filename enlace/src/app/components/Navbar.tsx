...
const Sidebar = () => {
  ...
  const [requestsOpen, setRequestsOpen] = useState(false); // State for the Requests dropdown
      ...
      <ul className={styles.sidebarLinks}>
        {links.map((item, index) => (
          <li key={index}>
           ...
           <span className='p-2 link' style={{ textDecoration: 'none' }} >{item.name}</span></Link>
            ...
            {isSidebarOpen && (
              <ul className={styles.dropdown}>
                {requestsOpen && item.dropdown && item.dropdown.map((subItem, subIndex) => (
                  <li key={subIndex}>
